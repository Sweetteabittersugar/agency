...
function addPanel(isOrch){var s=mkPanel(isOrch);panels.push(s);buildPanelDOM(s);curPage=Math.floor((panels.length-1)/perPage);refreshUI();return s}
...
function refreshUI(){// 面板不足时自动降级布局
if(panels.length<perPage&&perPage>1){perPage=panels.length>=4?4:panels.length>=2?2:1;curPage=0;grid.className='grid g'+perPage;try{localStorage.setItem('agency_layout',perPage)}catch(e){}}
...
function saveAllConvos(immediate){
  if(immediate){if(_saveTimer){clearTimeout(_saveTimer);_saveTimer=null}_doSaveConvos();return}
  if(_saveTimer)clearTimeout(_saveTimer);
  _saveTimer=setTimeout(function(){_saveTimer=null;_doSaveConvos()},200);
}
function _doSaveConvos(){
  // 1. 把面板数据合并进 conversations（匹配ID则更新，否则新增）
  panels.forEach(function(p){if(p.currentConvo.messages.length===0)return;if(!p.currentConvo.title||p.currentConvo.title==='新对话'){var f=p.currentConvo.messages.find(function(m){return m.role==='user'});if(f)p.currentConvo.title=f.content.slice(0,40)}var idx=conversations.findIndex(function(c){return c.id===p.currentConvo.id});var entry={id:p.currentConvo.id,title:p.currentConvo.title,messages:p.currentConvo.messages.slice(),sessionId:p.currentConvo.sessionId};if(idx>=0){conversations[idx]=entry}else{conversations.unshift(entry)}});
  // 2. 去重（同ID只保留第一个）
  var seen={};conversations=conversations.filter(function(c){if(!c.id||seen[c.id])return!1;seen[c.id]=!0;return c.messages&&c.messages.length>0});
  try{localStorage.setItem('agency_convos',JSON.stringify(conversations))}catch(e){}
  if(panels.length>0&&panels[0].currentConvo.sessionId){try{localStorage.setItem('agency-session-id',panels[0].currentConvo.sessionId)}catch(e){}}
  renderHistory();
}
...
/* ── 会话 Fork ── */
function addForkButton(msgEl,msgIndex,sessionId){if(!sessionId)return;var btn=document.createElement('button');btn.className='fork-btn';btn.title='从这里分叉出新对话';btn.innerHTML='⑂';btn.style.cssText='position:absolute;top:2px;right:2px;padding:1px 5px;background:transparent;border:1px solid var(--border);border-radius:3px;color:var(--muted);cursor:pointer;font-size:10px;opacity:0;transition:opacity 0.15s;';msgEl.style.position='relative';msgEl.appendChild(btn);msgEl.addEventListener('mouseenter',function(){btn.style.opacity='1'});msgEl.addEventListener('mouseleave',function(){btn.style.opacity='0'});btn.addEventListener('click',function(e){e.stopPropagation();var label=prompt('分叉标签（可选）：','分叉 '+new Date().toLocaleTimeString());if(label===null)return;api.post('/api/sessions/fork',{session_id:sessionId,fork_point:msgIndex,label:label}).then(function(d){if(d.ok){showToast('已分叉: '+d.fork_label);addPanel()}else showToast('分叉失败: '+(d.error||''),!0)})})}
...
function initSession(){
  var savedId = localStorage.getItem('agency-session-id');
  if(!savedId||typeof panels==='undefined'||panels.length===0) return;
  api.get('/api/sessions/'+savedId).then(function(data){
    if(!data.events||data.events.length===0) return;
    restoreSession(panels[0], data.events);
  }).catch(function(){});
}
// 延迟执行，等 app.js 初始化 panels 后再跑
setTimeout(initSession, 600);

function restoreSession(p, events){
  if(!p.dom.messages) return;
  if(p.dom.empty) p.dom.empty.style.display='none';
  events.forEach(function(evt){
    if(evt.type==='user_message'){
      var task = evt.data.task||'';
      addMsg(p, 'user', task);
      p.currentConvo.messages.push({role:'user',content:task});
    } else if(evt.type==='agent_response'){
      var response = evt.data.response||'';
      var bubble = addMsg(p, 'assistant', typeof renderMD==='function'?renderMD(response):response);
      if(typeof renderMD==='function') highlightCode(bubble);
      p.currentConvo.messages.push({role:'assistant',content:response});
    } else if(evt.type==='route_decision'){
      if(evt.data.agent&&evt.data.agent!=='auto'){
        p._lastAgent=evt.data.agent;
        p.dom.route.innerHTML='<span style="color:var(--accent);font-size:9px">📌 '+escHtml(evt.data.agent)+'</span>';
      }
    }
  });
