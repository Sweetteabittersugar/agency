...
function refreshUI(){var total=Math.ceil(panels.length/perPage),start=curPage*perPage;grid.className='grid g'+perPage;var gb=$('gridBtn');if(gb)gb.textContent='⊞';if(total>1){pageBar.style.display='flex';var pn=$('pgNum');if(pn)pn.textContent=(curPage+1)+'/'+total;var pp=$('pgPrev');if(pp)pp.disabled=curPage<=0;var pn2=$('pgNext');if(pn2)pn2.disabled=curPage>=total-1}else{pageBar.style.display='none'}panels.forEach(function(p,i){p.dom.wrapper.classList.toggle('on',i>=start&&i<start+perPage)});var sum=$('summary');if(sum)sum.textContent=panels.length+'窗'+(total>1?' '+(curPage+1)+'/'+total:'')}
...
(function initSession(){
  ...
  if(!savedId||panels.length===0) return;
