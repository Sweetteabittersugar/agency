...
logging.basicConfig(
    ...
    handlers=[
        logging.handlers.RotatingFileHandler(
            str(PROJECT_ROOT / "maestro" / "agency.log"),
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if length > 0:
            ...
            if not ct.startswith("application/json"):
                self.send_json(
                    {
                        "error": "请求体必须是 JSON 格式。请在请求头中设置 Content-Type: application/json"
                    },
                    400,
                )
                return
            ...
            for enc in ("utf-8", "gbk", "latin-1"):
                ...
                self.send_json(
                    {"error": "JSON 请求体格式无效。请检查 JSON 语法是否正确（引号、逗号、括号）"},
                    400,
                )
                return
        ...
        if tool_name:
            ...
            decision, risk, reason = check_tool_permission(
                tool_name, body.get("args", body), trust_mode
            )
            if decision == "deny":
                self.send_json(
                    {
    ...
    def send_json(self, data, code=200):
        ...
        self.send_header(
            ...
            "default-src 'self'; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline'; connect-src 'self' ws: wss:; require-trusted-types-for 'script'",
        )
        self.end_headers()
