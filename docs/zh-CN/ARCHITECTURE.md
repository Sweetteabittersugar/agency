...
everythingclaudecode 不是全家桶。它是四层结构，每层独立可用：
...
19 个 Agent 装在 `agents/` 目录里，但当前任务只需要 `coder` -- 只有 `coder.md` 的 prompt 被注入到 Claude Code 上下文。这就是为什么"多"不等于"慢"。
...
│   ├── planner.md         # 架构规划
│   ├── general-worker.md  # 通用任务
│   ├── webnovel-writer.md # 小说创作
│   ├── cost-analyst.md    # 费用分析
│   ├── doc-updater.md     # 文档更新
...
1. 创建 `agents/xxx.md`（遵循 YAML frontmatter + 正文格式）
