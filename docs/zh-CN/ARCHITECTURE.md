...
第 1 层：Agent 矩阵     ← 32 个 Agent，关键词自动路由
...
32 个 Agent 按 L3/L2/L1/L0 四层装在 `agents/` 目录里，但当前任务只需要 `coder` -- 只有 `coder.md` 的 prompt 被注入到 Claude Code 上下文。这就是为什么"多"不等于"慢"。
...
├── docs/GETTING-STARTED.md     # 5 分钟入门
├── install.sh / install.ps1  # 安装脚本
│
├── agents/                # 32 个 Agent（L3/L2/L1/L0 四层）
